...
歌曲列表会保留 QQ 官方目录中的 VIP、试听和不可用状态。VIP 要求优先于试听标识；
试听范围读取官方 `file.try_begin` / `file.try_end` / `file.size_try` 字段，完整音频大小
明确全部为零时标为不可用，但不会在没有明确依据时直接宣称“无版权”。
...
```text
https://cdn.jsdelivr.net/gh/DancingMusic/MusicConnect-QQMusic-Account@v0.3.1/dist/index.js
```
